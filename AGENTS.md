...
- Translations are generated via `grunt`/`wp i18n` during build. The `POT-Creation-Date` header is pinned (`updateTimestamp: false` in `Gruntfile.js`) so the `.pot` only changes when actual strings change — a clean checkout + build leaves the working tree clean.
